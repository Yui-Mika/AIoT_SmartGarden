...
export default function AboutClient() {
  ...
  useGSAP(() => {
    ...
    // 2. Hiệu ứng Wireframe-to-Glass khi cuộn
    const cards = gsap.utils.toArray(".wireframe-card");
  ...
  return (
    <main
      ref={containerRef}
      className="flex flex-col opacity-0 relative bg-transparent"
      style={{ minHeight: "100dvh" }}
    >
      {/* ── Ambient Light Orbs ── */}
      <div className="pointer-events-none fixed inset-0 z-[-1] overflow-hidden">
        <div className="absolute top-1/4 left-1/4 w-[400px] h-[400px] rounded-full blur-[100px] animate-soft-float" style={{ background: "rgba(34,211,238,0.03)" }} />
        <div className="absolute bottom-1/4 right-1/4 w-[500px] h-[500px] rounded-full blur-[120px] animate-soft-float" style={{ background: "rgba(16,185,129,0.02)", animationDelay: "-3s" }} />
      </div>
      ...
      <section className="container-app py-16">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {PILLARS.map(({ icon: Icon, iconColor, iconBg, title, desc }) => (
            <div
              key={title}
              className="wireframe-card p-7 rounded-2xl relative overflow-hidden transition-all"
              style={{
      ...
      >
        <div className="container-app py-16">
          ...
          <div
            className="wireframe-card relative overflow-hidden rounded-2xl p-7 md:p-10 transition-all"
            style={{
              background: "transparent",
              border: "1px dashed rgba(34, 211, 238, 0.3)",
              boxShadow: "none"
          ...
          >
            ...
            <div className="relative z-10 grid gap-4 sm:grid-cols-2">
              {TECH_STACK.map(({ icon: Icon, color, label, desc }, i) => (
                <div
                  key={label}
                  className="wireframe-card flex items-start gap-4 rounded-xl p-4 transition-all"
                  style={{
                    background: "transparent",
